        ...
        where TCommandInfo : CommandInfo<TAliasInfo>, new()
    {
        ...
        public virtual async Task HandleCallbackAsync(CallbackQuery callback)
        {
            ...
            logger.LogInformation(
                "callbackSplittedData: {0}\ncallbackDataType: {1}\ncallbackDataParams: {2}",
                string.Join(",", callbackSplittedData),
                callbackDataType,
                string.Join(",", callbackDataParams)
            );
            ...
            callbackInfo?.Handler.Invoke(moduleInstance, callbackParams.MatchParamTypes(callbackInfo.Handler));
